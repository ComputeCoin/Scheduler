...
\begin{document}
	...
	This proposed architecture can handle both dynamic agent population and state space when processing diverse systems. It can also handle static resource space by default. This architecture observes the total time of a user being in system as the only metric for scheduling optimization, by reducing other metrics to be about time. From this perspective, the PANDA is simpler and more efficient than traditional static schedulers. It can counter greedy scheduling algorithm to a more extensive degree, which in return gives novel insight into general scheduling processes.
		...
		% Process:
		% 	a) Submission:
		% 	 	-User fills in parameters for resource constraints and submits job.
		%		-Parameter constraints are mapped to a real-valued vector.
		%		-Parameter vector is classified as being a certain consumer type (some 
		%		 positive integer we refer to as the consumer's class label).
		% 	b) Construction:
		%		-Use the class label of the parameter vector and policy advisor outputs 
		%                the actor network parameters for the consumer's representative 
		%		 scheduling agent.
		% 	c) Allocation:
		%		-The consumer's agent then selects the resources that meet the  
		%        	 specification
		% 	d) Consumption:
		%		-The consumer uses its allocation of resources for whatever purposes it 
		%                might serve them.
		\begin{enumerate}
			%item[(a)] \mbox{\emph{Flow Rate (throughput)}} \\
			%\item(a) $\mathbf{Submission}$]\\
		...
		\subsubsection{Agent Model}
			...
			\begin{enumerate}
				\item[(A)] \mbox{$\mathbf{Catch}$}\\
				 The agent acquires an available resource to be assigned to its consumer. Caught resources are held until consumed or a \emph{release} action is taken. Catch requests are submitted as a Boolean vector, with each component corresponding to an item in the pool of observed resources.
				\item[(B)] \mbox{$\mathbf{Release}$}\\
				...
				\item[(C)] \mbox{$\mathbf{Wait}$}\\
				T he agent delays consumption of resources until the subsequent time step. Wait requests are submitted as a Boolean scalar, as only one wait action may be performed per time step.
			\end{enumerate}
	...
	\section{Training}
		...
		\begin{enumerate}
			...
			\item[(A)] \mbox{$\mathbf{Flow \hspace{0.05in} Rate \hspace{0.05in} (Throughput)}$} \\
			...
			\item[(B)] \mbox{$\mathbf{System \hspace{0.05in} Utilization}$} \\
